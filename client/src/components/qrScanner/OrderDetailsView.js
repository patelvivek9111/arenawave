...
const OrderDetailsView = ({
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4 text-center">Order Details</h2>
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-gray-900">Order #{scannedOrder.order_id}</h3>
            ...
            <div className="text-right">
              ...
              <p className="text-2xl sm:text-3xl font-bold gradient-text">₹{scannedOrder.total_price}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 border-b border-gray-100">
              <span className="text-sm text-gray-600 mb-1 sm:mb-0">Name</span>
              <span className="font-medium text-gray-900">{scannedOrder.customer_name}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 border-b border-gray-100">
              <span className="text-sm text-gray-600 mb-1 sm:mb-0">Email</span>
              <a 
                href={`mailto:${scannedOrder.email}`}
                className="font-medium text-primary-600 hover:text-primary-700 break-all flex items-center"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2">
              <span className="text-sm text-gray-600 mb-1 sm:mb-0">Phone</span>
              <a 
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <a
              ...
              className="flex items-center justify-center px-4 py-3 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition duration-300 border border-blue-200 min-h-[44px]"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center px-4 py-3 bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition duration-300 border border-purple-200 disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px]"
            >
              <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex-1">
                <p className="font-medium text-gray-900">FM Radio Earwing</p>
                <p className="text-sm text-gray-600">₹500 per unit</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600">Quantity</p>
                <p className="font-bold text-gray-900">{scannedOrder.quantity}</p>
              </div>
              <div className="text-right ml-4">
                <p className="text-sm text-gray-600">Subtotal</p>
                <p className="font-bold text-primary-600">₹{scannedOrder.total_price}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 border-b border-gray-100">
              <span className="text-sm text-gray-600 mb-1 sm:mb-0">Subtotal</span>
              <span className="font-medium text-gray-900">₹{scannedOrder.total_price}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2">
              <span className="text-sm font-semibold text-gray-900">Total Amount</span>
              <span className="text-lg font-bold text-primary-600">₹{scannedOrder.total_price}</span>
            </div>
          </div>
        </div>
            ...
            {scannedOrder.status === 'Processing' && (
              <div className="flex items-start">
                ...
                <div className="ml-4 flex-1">
                  ...
                  <p className="text-xs text-gray-600 mt-1">Order is being processed</p>
                </div>
              </div>
            )}
            ...
            {scannedOrder.status === 'Pending' && (
              <div className="flex items-start">
                ...
                <div className="ml-4 flex-1">
                  ...
                  <p className="text-xs text-gray-600 mt-1">Order is pending fulfillment</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto">
              {scannedOrder.notes.map((note, index) => (
                <div key={index} className={`p-3 rounded-lg ${note.is_internal ? 'bg-yellow-50 border border-yellow-200' : 'bg-gray-50 border border-gray-200'}`}>
                  <div className="flex items-start justify-between mb-1">
                    <p className="text-sm font-medium text-gray-900">{note.added_by}</p>
        ...
        {scannedOrder.status_history && scannedOrder.status_history.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
            <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-4">Status History</h4>
            <div className="space-y-3">
                ...
                <div key={index} className="flex items-start border-l-2 border-gray-200 pl-4">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">
                      Changed to <span className="text-primary-600">{history.status}</span>
                    </p>
                    <p className="text-xs text-gray-600 mt-1">
                      by {history.changed_by} • {new Date(history.changed_at).toLocaleString()}
                    </p>
                    {history.note && (
                      <p className="text-xs text-gray-500 mt-1 italic">"{history.note}"</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        {/* Action Buttons */}
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 space-y-3">
          <button
            onClick={onViewCustomerHistory}
            disabled={historyLoading}
            className="w-full bg-indigo-600 text-white py-2.5 px-4 min-h-[44px] rounded-lg text-sm font-medium hover:bg-indigo-700 transition duration-300 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {historyLoading ? (
              <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="bg-blue-600 text-white py-2.5 px-4 min-h-[44px] rounded-lg text-sm font-medium hover:bg-blue-700 transition duration-300 flex items-center justify-center"
            >
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
            ...
            <button
              ...
              className="bg-purple-600 text-white py-2.5 px-4 min-h-[44px] rounded-lg text-sm font-medium hover:bg-purple-700 transition duration-300 flex items-center justify-center"
            >
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
